...
Alerts are used highlight some aspect of the client's care that needs action or attention from the team.

These views groups data items that relate to a client's alert details such as the alert name, priority, names of teams members who are notified about the alert, and the team member who created the alert.
...
| CLOSEDBY|The identity of the user who who closed the alert.  | Character| 256|YES|
...
| CREATEDBY | USERS_V1_VIEW | Created-by joins to a user.<br /> A user is assoicated withs to zero-to-many alerts. |
| CLOSEDBY | USERS_V1_VIEW| Closed-by joins to one user.<br /> A user is assoicated with to zero-to-many closed alerts. |
| ALERTID | ALERT_NOTIFICATIONS_V1_VIEW| An alert is assoicated with zero-to-many notifications. |
| ALERTID | ALERT_COMMENTS_V1_VIEW| An alert is assoicated with zero-to-many comments. |
...
| Attribute | Joins to |Cardinality |
| :-------------- | :------ |:------ |
| ALERTID | ALERTS_V1_VIEW| Alertid joins to an alert.<br/> An alert is assoicated with zero-to-many notifications. |
